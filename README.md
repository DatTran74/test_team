...
This repository includes a workflow at `.github/workflows/deploy-gh-pages.yml`.

- Trigger: push to `develop` branch (or run manually from GitHub Actions tab)
- Output: static site from `dist/gh-pages`
- URL format: `https://<github-username>.github.io/<repository-name>/`

Notes:

- GitHub Pages deploys only the frontend static build.
- Teams local runtime (`src/index.ts`, port `3978`) is a Node.js server and is not hosted on GitHub Pages.
